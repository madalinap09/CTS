...
public class Main {
	...
	public static int transformNumber(int nr) {
		if(nr % 2 == 0) {
			return nr / 2;
		}
		else {
	...
	public static void main(String[] args) {
		...
		List<Integer> newList = 
				list.stream().distinct().filter(e -> e < 8).sorted().collect(Collectors.toList());
		...
		String result = 
				stringList.stream().filter(s -> s.length() > 2).collect(Collectors.joining(", "));
		...
		list.stream().map(Main::transformNumber).forEach(System.out::println);
